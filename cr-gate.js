...
(() => {
  ...
  function scheduleGate(){
    if(!localStorage.getItem(LS.FIRST)) localStorage.setItem(LS.FIRST, String(now()));
    const first = parseInt(localStorage.getItem(LS.FIRST)||'0',10);
    const delta = now()-first;
    if (delta >= FIVE_MIN) show($('#cr-auth-overlay'));
    else setTimeout(()=>show($('#cr-auth-overlay')), FIVE_MIN-delta);
  }
